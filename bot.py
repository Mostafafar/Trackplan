...
def get_students_list_keyboard():
    ...
    for student in students[:15]:  # حداکثر ۱۵ دانش‌آموز
        ...
        # استفاده از نام کامل دانش‌آموز در دکمه
...
async def handle_student_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # بهتر است از دکمه‌ها استفاده کنیم اما یک راه حل جایگزین:
    # استخراج آیدی دانش‌آموز از دکمه
    if "🎓" in text:
        try:
            # استخراج نام دانش‌آموز از متن دکمه
            # فرمت دکمه: "🎓 نام دانش‌آموز - پایه - زمان"
            parts = text.split(" - ")
            if len(parts) < 1:
                await update.message.reply_text(
                    "❌ فرمت دکمه نامعتبر است.",
                    reply_markup=get_students_list_keyboard()
                )
                return STUDENT_DETAILS
            
            student_name_full = parts[0].replace("🎓 ", "").strip()
